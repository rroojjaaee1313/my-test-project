...
# 連動選單 (必須放在 form 外，選擇後網頁才會即時刷新區域清單)
st.subheader("📍 物件位置")
...
with st.form("pro_form_final_v17"):
    ...
    with s1:
        ...
        c_build = st.text_input("總建坪", placeholder="輸入數字")
        c_age = st.text_input("屋齡", placeholder="輸入數字")
    with s2:
        c_inner = st.text_input("室內坪數 (主+附)", placeholder="輸入數字")
...
if submitted and model:
    with st.spinner("🕵️ 樂福導師正在跨平台偵察中..."):
        try:
            ...
            開價：{c_price}萬
            任務：執行全網行情比對、分析該樓層與室內占比競爭力、給予承辦人 {c_agent} 談價建議。
            * 禁止生成假網址。
            """
