...
# 核心資訊表單
with st.form("pro_form_v2026_final"):
    ...
    # 樓層欄位：併入地址組合
    c_floor = st.text_input("樓層 (地址一部分)", placeholder="例如：15樓、3樓之1")
    ...
    c1, c2 = st.columns(2)
    with c1:
        # 使用 text_input 確保初始狀態為空白，點擊即可輸入
        c_land = st.text_input("地坪", placeholder="輸入坪數")
        c_build_total = st.text_input("總建坪", placeholder="輸入坪數")
        c_age = st.text_input("屋齡 (年)", placeholder="輸入數字")
    with c2:
        c_build_inner = st.text_input("室內坪數 (主+附)", placeholder="輸入坪數")
        c_width = st.text_input("面寬 (米)", placeholder="輸入數字")
        c_elevator = st.selectbox("電梯", ["有", "無"])
        c_road = st.text_input("路寬 (米)", placeholder="輸入數字")
        
    c_price = st.text_input("開價 (萬)", placeholder="輸入價格")
...
if submitted and model:
    with st.spinner("🕵️ 樂福導師正在跨平台偵察中..."):
        try:
            ...
            開價：{c_price}萬
            任務：分析行情、樓層競爭力、室內占比優勢，指導承辦人 {c_agent} 議價開發。
            """
