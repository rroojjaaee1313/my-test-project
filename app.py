...
with col_in:
    with st.form("pro_form"):
        ...
        with c1_addr:
            city = st.selectbox("縣市", options=list(TAIWAN_DISTRICTS.keys()), index=3) # 預設台中
        with c2_addr:
            district = st.selectbox("區域", options=TAIWAN_DISTRICTS[city])
        
        road_name = st.text_input("路段名稱", placeholder="東榮路、大里路")
        ...
        with c1:
            ...
            c_build_total = st.number_input("總建坪", value=65.0)
            c_age = st.number_input("屋齡", value=15)
        with c2:
            c_build_inner = st.number_input("室內淨坪", value=55.0)
            c_width = st.number_input("面寬 (米)", value=5.0)
    ...
    with col_res:
        with st.spinner("分析中..."):
            try:
                ...
                prompt = f"""
                你是樂福導師，精簡回覆：
                物件：{full_loc} {c_name} (屋齡{c_age}年/地{c_land}/總建{c_build_total}/內淨{c_build_inner}/{c_elevator}/面寬{c_width}m)
                價格：{c_price}萬 (單價{round(c_price/c_build_total,2)}萬)
                
                內容：
                1.【行情】比對相似活案。
                2.【評估】內淨比{inner_pct}%與規格亮點。
                ...
                st.divider()
                st.link_button("🌐 開啟 5168 精準搜尋", f"https://house.5168.com.tw/list?keywords={full_loc}+{c_name}")
