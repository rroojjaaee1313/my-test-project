...
with st.form("pro_form_final_v2026"):
    ...
    st.divider()
    st.subheader("📏 實戰規格 (欄位完全清空，無預設 0)")
    c1, c2 = st.columns(2)
    with c1:
        # 使用 text_input 模擬數字，達成「完全空白」效果
        c_land = st.text_input("地坪", placeholder="請輸入地坪")
        c_build_total = st.text_input("總建坪", placeholder="請輸入總建")
        c_age = st.text_input("屋齡 (年)", placeholder="請輸入屋齡")
    with c2:
        c_build_inner = st.text_input("室內坪數 (主+附)", placeholder="請輸入坪數")
...
if submitted and model:
    with st.spinner("🕵️ 樂福導師正在跨平台偵察中..."):
        try:
            ...
            價格：{c_price}萬
            任務：執行行情比對、分析該樓層與室內占比競爭力、給予承辦人 {c_agent} 談價指導。
            * 禁止生成假網址。
            """
